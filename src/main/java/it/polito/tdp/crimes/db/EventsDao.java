...
public class EventsDao {
	...
	public List<String>getVertici(String categoria, int mese){
		String sql = "SELECT DISTINCT e.offense_type_id FROM events e WHERE e.offense_category_id = ? AND MONTH(e.reported_date) = ?";
	...
	public List<Adiacenza>getArchi(String categoria, int mese){
		String sql = "SELECT e1.offense_type_id AS v1, e2.offense_type_id AS v2, COUNT(e1.neighborhood_id) AS peso FROM events e1, events e2 WHERE e1.offense_type_id > e2.offense_type_id AND e1.offense_category_id =? AND e1.offense_category_id = e2.offense_category_id AND MONTH(e1.reported_date) =? AND MONTH(e1.reported_date) = MONTH(e2.reported_date) AND e1.neighborhood_id = e2.neighborhood_id GROUP BY e1.offense_type_id, e2.offense_type_id";
